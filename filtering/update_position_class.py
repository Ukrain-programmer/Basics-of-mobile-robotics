...
class PositionUpdate:
    ...
    def path_predicition(self, newcameramtx, dist,thymio,aruco_dict,aruco_params):
        ...
        if robot_detected:
            ...
            center, projected_points, tvec_marker, rvec_marker ,self.marker_centers,marker_corners_3d= Image_EKF.process_marker(corners, marker_id, 5, newcameramtx, dist,self.marker_centers)
            ...
            if not self.initial or self.kidnapped:
                ...
                self.odom_tvec, self.odom_rvec, center_odom, self.period,self.odom_centers,self.yaw_odom = update_odom_position(self.odom_tvec, self.marker_corners_3d, newcameramtx, dist, self.period,self.odom_centers,self.kf.X[2],self.initial_tz,False,self.kf.U,self.kf.Z)
                self.initial=True
                self.kf.theta=0
                end_time = time.time()
            ...
            self.kf.U=self.thymio.get_speed()
            self.odom_tvec, self.odom_rvec, center_odom, self.period,self.odom_centers ,self.yaw_odom= update_odom_position(self.odom_tvec, self.marker_corners_3d, newcameramtx, dist, self.period,self.odom_centers,self.yaw_odom,self.initial_tz,True,self.kf.U,self.kf.Z)
            self.kf.theta=yaw_odom
            end_time = time.time()
            self.period = end_time - current_time
        if self.initial:

...
class AStarNavigation:
    ...
    def algo(self):
        ...
        open_set = []
        heappush(open_set,
                 (self.heuristic(self.new_start, self.new_goal), 0, self.new_start))  # (f_cost, g_cost, position)
            ...
            for neighbor in neighbors:
                ...
                if (0 <= neighbor[0] < self.map_grid.shape[0]) and (0 <= neighbor[1] < self.map_grid.shape[1]):
                    if self.map_grid[neighbor[0], neighbor[1]] != -1 and neighbor not in explored:
                        ...
                        tentative_g_cost = current_g_cost + 1 + self.map_grid[
                            neighbor[0], neighbor[1]]  # todo remove if diag
